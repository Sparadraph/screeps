        ...
        for(var r in room.memory.market) {
            ...
            if(terminals.length > 0 && terminals[0].store[r] > data.max){
                ...
                Memory.market.b_offers[r].forEach(function(offer) {
                    var amount = Math.min(offer.remainingAmount, terminals[0].store[r] - data.max, 2000);
            ...
            if(terminals.length > 0 && terminals[0].store[r] < data.min) {
                ...
                Memory.market.s_offers[r].forEach(function(offer) {
                    var amount = Math.min(offer.remainingAmount, data.min - terminals[0].store[r], 1000);
                    var energy_cost = Game.market.calcTransactionCost(amount, offer.roomName, room.name);
